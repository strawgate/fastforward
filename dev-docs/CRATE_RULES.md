...
| Pure seam Kani boundary status tracked in `dev-docs/verification/kani-boundary-contract.toml` | CI script: `python3 scripts/verify_kani_boundary_contract.py` |
| **No raw payload injection.** Source metadata (`_source_path`, `_source_id`, etc.) must be attached as Arrow columns post-scan, never injected into raw bytes pre-scan. The existing `inject_source_path_metadata` is deprecated (#1615) and must not be extended. New metadata fields use the `_resource_*` column pattern from the OTLP path. | Code review |
