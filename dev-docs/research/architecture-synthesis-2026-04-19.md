...
`FileReader` in `crates/ffwd-io/src/tail/reader.rs` manages open/read/truncate/eof/evict/offset mutation in one large struct. `EofState` reducer exists but models only EOF + error backoff, not full per-file lifecycle.
