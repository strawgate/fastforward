...
// ---------------------------------------------------------------------------
// Collision avoidance — cars never overlap
// ---------------------------------------------------------------------------

describe('collision avoidance', function () {
  it('trailing car stops before hitting car ahead', function () {
    const sep = 18;
    const rs = createRenderState({ maxSpeed: 10, accel: 10, decelDist: 5, snapDist: 0.5, minSep: sep });
    // Car B is ahead at x=30, stationary
    rs.addCar('b', 30, 0, 0, 0);
    // Car A is behind at x=0, fast, chasing toward x=50 (past B)
    rs.addCar('a', 0, 0, 0, 10);
    rs.pushTarget('a', 50, 0, 1, 'flow');
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const b = rs.getCar('b');
      const gap = dist(a, b);
      assert.ok(gap >= sep - 1, 'cars should not overlap (gap=' + gap.toFixed(1) + ' at step ' + i + ')');
    }
  });

  it('car never moves backward due to collision avoidance', function () {
    const rs = createRenderState({ maxSpeed: 5, accel: 5, decelDist: 10, snapDist: 0.5, minSep: 18 });
    rs.addCar('a', 0, 0, 0, 5);
    rs.addCar('b', 10, 0, 0, 0);
    rs.pushTarget('a', 50, 0, 1, 'flow');
    ...
    for (let i = 0; i < 30; i++) {
      rs.step();
      const a = rs.getCar('a');
      assert.ok(a.x >= prevX - 0.01, 'car should never move backward (x=' + a.x.toFixed(1) + ', prev=' + prevX.toFixed(1) + ')');
      prevX = a.x;
    }
  });

  it('cars on perpendicular paths do not block each other', function () {
    const rs = createRenderState({ maxSpeed: 5, accel: 5, decelDist: 10, snapDist: 0.5, minSep: 18 });
    // Car A goes right, car B goes down — they pass near (50, 50) but shouldn't block
    rs.addCar('a', 0, 50, 0, 5);
    rs.pushTarget('a', 100, 50, 1, 'flow');
    rs.addCar('b', 50, 0, 90, 5);
    rs.pushTarget('b', 50, 100, 1, 'flow');

    runSteps(rs, 50);
    const a = rs.getCar('a');
    const b = rs.getCar('b');
    // Both should have made significant progress
    assert.ok(a.x > 50, 'car A should have passed midpoint (x=' + a.x.toFixed(1) + ')');
    assert.ok(b.y > 50, 'car B should have passed midpoint (y=' + b.y.toFixed(1) + ')');
  });
});
