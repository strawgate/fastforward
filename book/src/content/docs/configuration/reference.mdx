...
| `encoding` | enum | No | Row encoding (`json`, `raw`, `text`, or `console`). |
| `framing` | enum | No | Stream framing format. |
| `tls` | object | No | Optional TLS client options (`ca_file`, `cert_file`, `key_file`, `insecure_skip_verify`). |
| `keepalive` | object | No | Optional TCP keepalive settings. |
| `timeout_secs` | integer | No | Optional connection timeout in seconds. |
| `retry` | object | No | Optional retry settings. |
| `batch` | object | No | Optional batch settings. |
...
| `encoding` | enum | No | Datagram row encoding (`json`, `raw`, `text`, or `console`). |
| `max_datagram_size_bytes` | integer | No | Optional maximum datagram payload size. |
