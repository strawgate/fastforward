            ...
            let result = (|| -> Result<(), ConfigError> {
                ...
                for (i, output) in pipe.outputs.iter().enumerate() {
                    ...
                    if output.output_type == OutputType::Elasticsearch {
                        ...
                        if let Some(pipe) = &output.pipeline
                            && pipe.trim().is_empty()
                        {
                            return Err(ConfigError::Validation(format!(
                                "pipeline '{name}' output '{label}': elasticsearch 'pipeline' must not be empty"
                            )));
                        }
                        if let Some(0) = output.max_bulk_bytes {
                            return Err(ConfigError::Validation(format!(
                                "pipeline '{name}' output '{label}': elasticsearch 'max_bulk_bytes' must be > 0"
                            )));
                        }
                    } else {
                    ...
                    if let Some(c) = output.compression.as_deref() {
                        match output.output_type {
                            ...
                            // ArrowIpc allows lz4/zstd/none and is validated above.
