...
fn write_json_value(arr: &dyn Array, row: usize, out: &mut Vec<u8>) -> io::Result<()> {
    match arr.data_type() {
        ...
        // Timestamp → "YYYY-MM-DDTHH:MM:SS.fffffffffZ" (fixes #1662)
        DataType::Timestamp(unit, _tz) => {
            if arr.is_null(row) {
                ...
                write_nanos_iso8601(out, nanos);
            }
        }
        // Date32 → "YYYY-MM-DDTOO:00:00.000000000Z" (days since epoch)
        DataType::Date32 => {
            if arr.is_null(row) {
                out.extend_from_slice(b"null");
            } else {
                let days = arr
                    ...
                    .value(row);
                let nanos = (days as i64).saturating_mul(86_400_000_000_000);
                write_nanos_iso8601(out, nanos);
            }
        }
        // Date64 → "YYYY-MM-DDTHH:MM:SS.fffffffffZ" (milliseconds since epoch)
        DataType::Date64 => {
            if arr.is_null(row) {
                out.extend_from_slice(b"null");
            } else {
...
/// Write an i64 nanosecond-since-epoch value as a quoted ISO 8601 UTC string.
///
/// Negative timestamps (pre-epoch) are clamped to epoch. This function is
/// the common formatting path shared by the Timestamp, Date32, and Date64
/// arms of [`write_json_value`].
fn write_nanos_iso8601(out: &mut Vec<u8>, nanos: i64) {
    let nanos_u = if nanos < 0 { 0u64 } else { nanos as u64 };
    let secs = nanos_u / 1_000_000_000;
    let frac = nanos_u % 1_000_000_000;
    let mut ts_buf = [0u8; 19];
    elasticsearch::write_unix_timestamp_utc_into(&mut ts_buf, secs);
    out.push(b'"');
    out.extend_from_slice(&ts_buf);
    out.push(b'.');
    // Write 9-digit zero-padded nanosecond fraction.
    let mut f = frac;
    let mut frac_buf = [0u8; 9];
    for i in (0..9).rev() {
...
mod write_row_json_tests {
    ...
    /// Regression for #1662: Arrow Timestamp columns must be emitted as ISO 8601
    /// strings, not as empty strings.
    #[test]
    fn write_row_json_timestamp_ns_emits_iso8601() {
        use arrow::array::TimestampNanosecondArray;

        // 2024-01-15T10:30:00.000000000Z = 1705314600_000000000 ns since epoch
        let ts_ns: i64 = 1_705_314_600_000_000_000;
        let arr = TimestampNanosecondArray::from(vec![ts_ns]);
        let batch = make_batch(vec![("event_ts", Arc::new(arr) as Arc<dyn Array>)]);
        let json = render(&batch, 0);
        let v: serde_json::Value = serde_json::from_str(&json).expect("must be valid JSON");
        let s = v["event_ts"].as_str().expect("event_ts must be a string");
        assert!(
            s.starts_with("2024-01-15T10:30:00."),
            "Timestamp must be ISO 8601, got: {s}"
        );
        assert!(s.ends_with('Z'), "Timestamp must end with Z, got: {s}");
    }

    /// Regression for #1662: Arrow Timestamp(Microsecond) columns must also be
    /// emitted as ISO 8601 strings.
    #[test]
    fn write_row_json_timestamp_us_emits_iso8601() {
        use arrow::array::TimestampMicrosecondArray;

        let ts_us: i64 = 1_705_314_600_000_000; // same instant in microseconds
        let arr = TimestampMicrosecondArray::from(vec![ts_us]);
        let batch = make_batch(vec![("event_ts", Arc::new(arr) as Arc<dyn Array>)]);
        let json = render(&batch, 0);
        let v: serde_json::Value = serde_json::from_str(&json).expect("must be valid JSON");
        let s = v["event_ts"].as_str().expect("event_ts must be a string");
        assert!(
            s.starts_with("2024-01-15T10:30:00."),
            "Timestamp(us) must be ISO 8601, got: {s}"
        );
    }

    /// Regression for #1662: Arrow Date32 columns must be emitted as ISO 8601
    /// date strings, not empty strings.
    #[test]
    fn write_row_json_date32_emits_iso8601() {
        use arrow::array::Date32Array;

        // 2024-01-15: days since Unix epoch = 19737
        let arr = Date32Array::from(vec![19737i32]);
        let batch = make_batch(vec![("date_col", Arc::new(arr) as Arc<dyn Array>)]);
        let json = render(&batch, 0);
        let v: serde_json::Value = serde_json::from_str(&json).expect("must be valid JSON");
        let s = v["date_col"].as_str().expect("date_col must be a string");
        assert!(
            s.starts_with("2024-01-15T"),
            "Date32 must be ISO 8601 with date prefix, got: {s}"
        );
    }
