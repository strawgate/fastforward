...
impl InputSource for OtlpReceiverInput {
    fn poll(&mut self) -> io::Result<Vec<InputEvent>> {
        ...
        // Drain all available decoded payloads without coalescing push input
        // into one oversized downstream unit.
...
mod tests {
    ...
    fn json_bytes_value_matches_protobuf_semantics() {
        ...
        assert_eq!(
            left.get(field_names::BODY)
                .and_then(serde_json::Value::as_str),
            Some("01020304")
        );
        assert_eq!(left.get(field_names::BODY), right.get(field_names::BODY));
    ...
    #[test]
    fn poll_drains_json_payloads_without_coalescing_or_reordering() {
        ...
        let events = receiver.poll().expect("poll succeeds");
        assert_eq!(
            events.len(),
            bodies.len(),
            "poll should drain all queued payloads without merging them"
        );

        for ((event, expected_body), expected_message) in events
            .into_iter()
            .zip(bodies.iter())
            .zip(["first", "second", "third"])
        {
            let InputEvent::Data {
                ...
                panic!("expected JSON lines data event");
            };
            assert_eq!(source_id, None);
            assert_eq!(accounted_bytes, expected_body.len() as u64);

            let text = String::from_utf8(bytes).expect("JSON lines must be UTF-8");
            let lines: Vec<&str> = text.lines().collect();
            assert_eq!(lines.len(), 1, "each payload should remain separate");
        ...
        assert!(
            receiver.poll().expect("second poll succeeds").is_empty(),
            "first poll should have drained all queued payloads"
        );
    }
