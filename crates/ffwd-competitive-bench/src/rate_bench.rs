//! Ingest benchmark: measures RSS memory and CPU usage for ffwd at
//! controlled event rates and at maximum throughput.
//!
//! This is a non-competitive benchmark — it only runs ffwd.
//!
...
//!   1. Writes a ffwd config that tails a temp file and forwards to the
//!      provided blackhole address.
//!   2. Spawns ffwd, then a writer thread (rate-limited or unlimited).
...
/// Port used for ffwd's diagnostics server during rate-bench runs.
...
///
/// * `ffwd_binary` — path to the ffwd binary  
...
pub fn run_rate_bench(
    ...
    for &eps in DEFAULT_EPS_LEVELS {
        ...
        match run_single_rate(ffwd_binary, bench_dir, blackhole_addr, eps) {
            Ok(r) => {
                eprintln!(
...
fn run_single_rate(
    ...
    // Create an empty file so ffwd can open it immediately.
    std::fs::write(&data_file, b"").map_err(|e| e.to_string())?;
    write_ffwd_config(&cfg_path, &data_file, blackhole_addr)?;

    // Spawn ffwd.
    let mut child = Command::new(ffwd_binary)
        .arg("run")
        .arg("--config")
        .arg(&cfg_path)
        ...
        .spawn()
        .map_err(|e| format!("failed to spawn ffwd: {e}"))?;

    let pid = child.id();

    // Brief pause so ffwd can open the file before we start writing.
...
pub fn print_rate_results_markdown(results: &[RateBenchResult]) {
    ...
    println!(
        "Resource usage sampled over {MEASURE_SECS}s steady-state at each ingest rate \
         (ffwd only, passthrough scenario).\n"
    );
