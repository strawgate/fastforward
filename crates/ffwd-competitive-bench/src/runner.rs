...
pub fn run_agent_docker(
    ...
    let container_name = format!("ffwd-bench-{}", agent.name());
...
/// Run ffwd under `perf record` for CPU profiling.
...
/// Run ffwd built with dhat-heap for memory profiling.
...
pub fn run_agent_dhat(
    ...
    if dhat_file.exists() {
        ...
        Err("dhat-heap.json not found — was ffwd built with --features dhat-heap?".to_string())
    }
}
