//! Competitive benchmark harness: ffwd vs vector vs fluent-bit vs filebeat
...
//!
//! Usage:
//!   cargo run -p ffwd-competitive-bench --release -- \[OPTIONS]
//!
...
//!   --dhat-binary PATH   ffwd binary built with --features dhat-heap
...
fn main() {
    ...
    if let Some(ref profile_dir) = args.profile_dir {
        ...
        // Find the ff binary.
        let ffwd_resolved = available.iter().find(|r| r.agent.name() == "ff");

        if let Some(resolved) = ffwd_resolved.filter(|r| r.binary.is_some()) {
            let binary = resolved.binary.as_ref().unwrap();
            let ffwd_agent = resolved.agent;
            ...
            if runner::perf_available() {
                ...
                match runner::run_agent_perf(ffwd_agent, binary, &ctx, &blackhole, profile_dir) {
                    Ok(perf_data) => {
            ...
            if let Some(ref dhat_binary) = args.dhat_binary {
                ...
                eprintln!(
                    "  SKIP: --dhat-binary not set (build with: cargo build --release --features dhat-heap -p ffwd)"
                );
            }
...
fn run_rate_bench_main(args: &Args) {
    ...
    let ffwd_binary = find_ffwd_binary().unwrap_or_else(|| {
        eprintln!(
    ...
    eprintln!("  binary: {}", ffwd_binary.display());
    eprintln!();
    ...
    let results = rate_bench::run_rate_bench(&ffwd_binary, bench_dir.path(), blackhole_addr);
...
/// then `PATH`.
fn find_ffwd_binary() -> Option<PathBuf> {
...
struct Args {
    ...
    /// Path to a ffwd binary built with --features dhat-heap.
...
impl Args {
    fn parse() -> Self {
        ...
        while i < args.len() {
            let arg = &args[i];
            if arg == "--help" || arg == "-h" {
                eprintln!("Usage: ffwd-competitive-bench [OPTIONS]");
                eprintln!();
                ...
                eprintln!("  --dhat-binary PATH   ffwd binary built with --features dhat-heap");
                eprintln!(
