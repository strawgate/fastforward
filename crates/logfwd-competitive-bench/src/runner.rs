...
pub struct BenchResult {
    ...
    pub iteration: usize,
    /// Time-series samples collected during the run (1/sec).
...
pub fn run_agent(
    ...
    // We need a way to call parse_stats from the thread. Since Agent isn't Send,
    // capture the stats_url and do a generic parse in the thread — agent-specific
    // parsing happens after we join.
    ...
    // Parse agent-specific stats from the raw HTTP bodies.
...
/// Raw sample with optional HTTP body for agent-specific parsing.
struct RawSample {
    sample: AgentSample,
...
fn collect_samples(
    ...
    while !stop.load(std::sync::atomic::Ordering::Relaxed) {
        ...
        // Process-level metrics from OS (universal fallback).
        let (rss, cpu_user, cpu_sys) = procfs_stats(pid);

        // Agent-specific stats via HTTP (if available).
...
/// Read RSS and CPU from /proc/{pid}/status and /proc/{pid}/stat (Linux only).
/// Returns (rss_bytes, cpu_user_ms, cpu_sys_ms).
